...
import { ProjectFolderPathConflictError, ProjectNotFoundError, ProjectValidationFailedError, } from './errors.js';
...
function buildProjectFolderPath(name, id) {
    return `${name}__${createShortId(id)}`;
}
function toProjectSummary(record) {
    return {
        id: record.id,
        name: record.name,
...
export function createProjectService(database, workspacePaths) {
