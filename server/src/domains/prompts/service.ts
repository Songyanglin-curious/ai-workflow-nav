...
import { type SqliteDatabase, runInTransaction } from '../../infra/db/index.js';
import { type WorkspacePaths, resolvePromptPath } from '../../infra/workspace/index.js';
import {
  PromptFileMissingError,
  PromptFilePathConflictError,
  PromptNotFoundError,
  PromptValidationFailedError,
} from './errors.js';
import { createPromptRepo, type PromptQuery, type PromptRecord } from './repo.js';
...
function assertPromptName(name: string): string {
...
function buildPromptFilePath(name: string, id: string): string {
...
async function readPromptContent(workspacePaths: WorkspacePaths, record: PromptRecord): Promise<string> {
...
export function createPromptService(database: SqliteDatabase, workspacePaths: WorkspacePaths) {
  const repo = createPromptRepo(database);

  return {
    listPrompts(query: PromptQuery = {}): PromptSummary[] {
      return repo.listPrompts(query).map(toPromptSummary);
    },

    async getPromptById(id: string): Promise<PromptDetail> {
    ...
    async createPrompt(input: CreatePromptInput): Promise<PromptDetail> {
    ...
    async updatePrompt(id: string, patch: UpdatePromptInput): Promise<PromptDetail> {
    ...
    async deletePromptById(id: string): Promise<{ deleted: true }> {
      const record = repo.getPromptById(id);
