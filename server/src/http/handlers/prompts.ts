...
import {
  presentPromptDeleteEnvelope,
  presentPromptDetailEnvelope,
  presentPromptListEnvelope,
} from '../presenter/prompts.js';
import {
  createPromptBodySchema,
  promptListQueryHttpSchema,
  promptParamsSchema,
  updatePromptBodySchema,
} from '../schema/prompts.js';
import type { PromptService } from '../../domains/prompts/service.js';

export interface RegisterPromptRoutesOptions {
...
function mapPromptError(error: unknown): HttpMappedError {
...
function replyWithPromptError(reply: FastifyReply, error: unknown): FastifyReply {
  const mappedError = mapPromptError(error);

  return reply.status(mappedError.statusCode).send(
    presentError(mappedError.code, mappedError.message, mappedError.statusCode === 422 ? mapValidationIssues(error) : undefined),
  );
}

export async function registerPromptRoutes(
  app: FastifyInstance,
  options: RegisterPromptRoutesOptions,
): Promise<void> {
  app.get('/api/prompts', async (request, reply) => {
    ...
    if (!parsedQuery.success) {
      return reply.status(422).send(
        presentError('VALIDATION_ERROR', '提示词列表查询参数校验失败。', mapValidationIssues(parsedQuery.error)),
      );
    }
  ...
  app.get('/api/prompts/:id', async (request, reply) => {
    ...
    if (!parsedParams.success) {
      return reply.status(422).send(
        presentError('VALIDATION_ERROR', '提示词路径参数校验失败。', mapValidationIssues(parsedParams.error)),
      );
    }
  ...
  app.post('/api/prompts', async (request, reply) => {
    ...
    if (!parsedBody.success) {
      return reply.status(422).send(
        presentError('VALIDATION_ERROR', '提示词创建请求校验失败。', mapValidationIssues(parsedBody.error)),
      );
    }
  ...
  app.patch('/api/prompts/:id', async (request, reply) => {
    ...
    if (!parsedBody.success) {
      return reply.status(422).send(
        presentError('VALIDATION_ERROR', '提示词更新请求校验失败。', mapValidationIssues(parsedBody.error)),
      );
    }
