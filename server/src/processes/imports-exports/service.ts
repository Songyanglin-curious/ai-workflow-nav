...
import { resolveSyncPath, type WorkspacePaths } from '../../infra/workspace/index.js';
...
export type ImportMode = 'rebuild';
...
type CsvColumnKind = 'text' | 'nullableText' | 'integer' | 'number';
...
interface TableSpec {
  ...
  insertSql: string;
  selectSql: string;
  columnKinds?: Partial<Record<string, CsvColumnKind>>;
}
...
function asCsvText(rows: Array<Record<string, unknown>>, columns: readonly string[]): string {
  const csvRows = [columns, ...rows.map((row) => columns.map((column) => String(row[column] ?? '')))];
  return `${stringifyCsvRows(csvRows)}\n`;
}

function normalizeImportedValue(column: string, value: string, kind: CsvColumnKind | undefined): unknown {
...
function buildInsertParameters(
  ...
  return Object.fromEntries(
    spec.columns.map((column) => [column, normalizeImportedValue(column, record[column], spec.columnKinds?.[column])]),
  );
}
...
function checkManifest(manifestText: string): SyncManifest {
  const parsedManifest = JSON.parse(manifestText) as unknown;

  if (
    typeof parsedManifest !== 'object' ||
    parsedManifest === null ||
    typeof (parsedManifest as { version?: unknown }).version !== 'number' ||
    typeof (parsedManifest as { exportedAt?: unknown }).exportedAt !== 'string' ||
    !Array.isArray((parsedManifest as { files?: unknown }).files) ||
    !(parsedManifest as { files: unknown[] }).files.every((fileName) => typeof fileName === 'string')
  ) {
    throw new ImportsExportsValidationError('manifest.json 内容不合法。');
  }

  const manifest: SyncManifest = {
    version: (parsedManifest as { version: number }).version,
    exportedAt: (parsedManifest as { exportedAt: string }).exportedAt,
    files: [...(parsedManifest as { files: string[] }).files],
  };
...
function assertRequiredFiles(manifest: SyncManifest, availableFileNames: Set<string>): void {
...
export function createImportsExportsProcess(database: SqliteDatabase, workspacePaths: WorkspacePaths) {
  return {
    async exportSync(): Promise<SyncExportResult> {
      await ensureDirectory(workspacePaths.syncDirectoryPath);

      for (const spec of requiredTableSpecs) {
        const rows = database.prepare<Record<string, unknown>>(spec.selectSql).all();
      ...
      await writeTextFile(
        resolveSyncPath(workspacePaths, syncManifestFileName),
    ...
    async importSync(mode: ImportMode): Promise<SyncImportResult> {
      if (mode !== 'rebuild') {
      ...
      const csvTexts = new Map<string, string>();
...
export type ImportsExportsProcess = ReturnType<typeof createImportsExportsProcess>;
