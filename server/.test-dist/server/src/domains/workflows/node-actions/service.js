...
import { MermaidNodeNotFoundError, ToolTargetNotFoundError, WorkflowNodeActionConflictError, WorkflowNodeActionNotFoundError, WorkflowNodeActionValidationFailedError, } from './errors.js';
import { createWorkflowNodeActionRepo } from './repo.js';
...
function assertMermaidNodeId(value) {
...
function toNodeActionItem(record) {
    return {
...
function getWorkflowNodeIds(record) {
...
function assertWorkflowExists(workflowRecord, workflowId) {
    if (!workflowRecord) {
        throw new WorkflowNotFoundError(workflowId);
    }
    return workflowRecord;
}
export function createWorkflowNodeActionService(database, options = {}) {
    ...
    function assertMermaidNodeExists(workflowRecord, mermaidNodeId) {
    ...
    function assertActionTargetExists(actionType, targetRef) {
    ...
    function syncNodeActionsInCurrentTransaction(workflowId) {
    ...
    return {
        ...
        createNodeAction(workflowId, input) {
            const workflowRecord = assertWorkflowExists(workflowRepo.getWorkflowById(workflowId), workflowId);
        ...
        updateNodeActionByWorkflowIdAndMermaidNodeId(workflowId, mermaidNodeId, patch) {
            ...
            const nextTargetRef = patch.targetRef !== undefined ? assertTargetRef(patch.targetRef) : currentRecord.targetRef;
            ...
            runInTransaction(database, () => repo.updateNodeActionByWorkflowIdAndMermaidNodeId(workflowId, normalizedMermaidNodeId, {
        ...
        deleteNodeActionByWorkflowIdAndMermaidNodeId(workflowId, mermaidNodeId) {
            ...
            runInTransaction(database, () => repo.deleteNodeActionByWorkflowIdAndMermaidNodeId(workflowId, normalizedMermaidNodeId));
            return {
                deleted: true,
            };
        },
        syncNodeActions(workflowId) {
