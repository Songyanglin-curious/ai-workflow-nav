...
import { createProjectViewConfigRepo, type ProjectNodeLayoutRecord, type ProjectViewportRecord } from './repo.js';

export interface PatchProjectNodeLayoutsItem {
  projectNodeId: string;
  positionX: number;
  positionY: number;
}

export interface PatchProjectNodeLayoutsInput {
  items: PatchProjectNodeLayoutsItem[];
}

export interface PatchProjectViewportInput {
  viewportX: number;
  viewportY: number;
  zoom: number;
}

export interface ProjectNodeLayoutItem {
  projectNodeId: string;
  positionX: number;
  positionY: number;
  createdAt: string;
...
export interface PatchProjectNodeLayoutsResult {
  updatedCount: number;
}
...
function assertProjectExists(projectId: string, exists: boolean): void {
  if (!exists) {
    throw new ProjectNotFoundError(projectId);
  }
}

function assertViewportPatchInput(input: PatchProjectViewportInput): void {
  if (input.zoom <= 0) {
    throw new ProjectViewConfigValidationFailedError('项目视角缩放必须大于 0。');
  }
}

function assertProjectNodeLayoutsPatchInput(items: PatchProjectNodeLayoutsItem[]): void {
...
function toProjectNodeLayoutItem(record: ProjectNodeLayoutRecord): ProjectNodeLayoutItem {
  return {
...
function toProjectViewport(record: ProjectViewportRecord): ProjectViewport {
  return {
...
export function createProjectViewConfigService(database: SqliteDatabase) {
  const repo = createProjectViewConfigRepo(database);

  return {
    getProjectNodeLayouts(projectId: string): ProjectNodeLayoutItem[] {
    ...
    patchProjectNodeLayouts(projectId: string, input: PatchProjectNodeLayoutsInput): PatchProjectNodeLayoutsResult {
      assertProjectExists(projectId, repo.getProjectById(projectId) !== undefined);
      ...
      const currentLayouts = new Map(
        repo.listProjectNodeLayoutsByProjectId(projectId).map((record) => [record.projectNodeId, record]),
      );

      runInTransaction(database, () => {
        for (const item of input.items) {
          const currentRecord = currentLayouts.get(item.projectNodeId);
          const nextRecord: ProjectNodeLayoutRecord = {
    ...
    getProjectViewport(projectId: string): ProjectViewport {
      assertProjectExists(projectId, repo.getProjectById(projectId) !== undefined);
    ...
    patchProjectViewport(projectId: string, input: PatchProjectViewportInput): ProjectViewport {
      assertProjectExists(projectId, repo.getProjectById(projectId) !== undefined);
      assertViewportPatchInput(input);

      const currentViewport = repo.getProjectViewportByProjectId(projectId);
      const now = createTimestamp();
      const nextRecord: ProjectViewportRecord = {
...
export type ProjectViewConfigService = ReturnType<typeof createProjectViewConfigService>;
