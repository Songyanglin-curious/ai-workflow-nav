...
import {
  ParentProjectNodeNotFoundError,
  ProjectNodeCycleDetectedError,
  ProjectNodeNotFoundError,
  ProjectNodeValidationFailedError,
} from './errors.js';
import {
  createProjectNodeRepo,
  type ProjectNodeEntityRecord,
  type ProjectNodeQuery,
  type ProjectNodeRecord,
  type ProjectNodeRelationRecord,
  type ProjectNodeWorkflowRecord,
} from './repo.js';
...
export interface ProjectNodeDetail extends ProjectNodeSummary {}
...
function assertNodeName(name: string): string {
...
function buildProjectNodeFolderPath(projectFolderPath: string, nodeName: string, nodeId: string): string {
  return `${projectFolderPath}/${nodeName}__${createShortId(nodeId)}`;
}

function toProjectNodeSummary(record: ProjectNodeRecord): ProjectNodeSummary {
  return {
    id: record.id,
...
function resolveNextSortOrder(
  repo: ReturnType<typeof createProjectNodeRepo>,
  projectId: string,
  parentNodeId: string | null,
...
function assertNoCycle(
  nodeId: string,
  parentNodeId: string | null,
  relations: ProjectNodeRelationRecord[],
): void {
  if (!parentNodeId) {
    return;
  }
...
export function createProjectNodeService(database: SqliteDatabase, workspacePaths: WorkspacePaths) {
  ...
  return {
    listProjectNodes(projectId: string, query: ProjectNodeQuery = {}): ProjectNodeSummary[] {
    ...
    getProjectNodeById(id: string): ProjectNodeDetail {
    ...
    async createProjectNode(projectId: string, input: CreateProjectNodeInput): Promise<ProjectNodeDetail> {
      ...
      const nodeRecord: ProjectNodeEntityRecord = {
        id: nodeId,
        projectId,
        name: nodeName,
      ...
      const relationRecord: ProjectNodeRelationRecord = {
        id: createUuidV7(),
        projectId,
      ...
      const workflowRecord: ProjectNodeWorkflowRecord | undefined =
        workflowId === null
          ? undefined
          : {
              id: createUuidV7(),
      ...
      try {
        runInTransaction(database, () => {
          ...
          deliberationsRepo.createDeliberationsRecord(
            createDeliberationsRecord(nodeId, `${nodeFolderPath}/deliberations`, now, createUuidV7()),
          );
          summaryRepo.createSummaryRecord(
            createSummaryRecord(nodeId, `${nodeFolderPath}/summaries`, now, createUuidV7()),
          );
        });
      } catch (error) {
    ...
    updateProjectNode(id: string, patch: UpdateProjectNodeInput): ProjectNodeDetail {
      ...
      const parentNodeId =
        patch.parentNodeId !== undefined ? patch.parentNodeId : currentRecord.parentNodeId;
      const nextWorkflowId =
        patch.workflowId !== undefined ? patch.workflowId : currentRecord.workflowId;
      ...
      const nextEntityRecord: ProjectNodeEntityRecord = {
        id: currentRecord.id,
        ...
        status:
          patch.status !== undefined ? assertProjectNodeStatus(patch.status) : currentRecord.status,
        folderPath: currentRecord.folderPath,
        createdAt: currentRecord.createdAt,
        updatedAt: createTimestamp(),
      };
      const nextSortOrder =
        patch.parentNodeId !== undefined && patch.sortOrder === undefined
...
export type ProjectNodeService = ReturnType<typeof createProjectNodeService>;
