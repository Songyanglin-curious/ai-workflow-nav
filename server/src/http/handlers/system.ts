...
import {
  presentProjectDeletionCheckEnvelope,
  presentProjectDeletionExecuteEnvelope,
  presentProjectNodeDeletionCheckEnvelope,
  presentProjectNodeDeletionExecuteEnvelope,
  presentWorkflowRuntimeDetailEnvelope,
  presentWorkflowRuntimeTriggerEnvelope,
} from '../presenter/projects.js';
import {
  presentInspectionRunEnvelope,
  presentSelfCheckEnvelope,
  presentStartupReportEnvelope,
  presentSyncExportEnvelope,
  presentSyncImportEnvelope,
} from '../presenter/system.js';
import {
  projectDeletionExecuteBodySchema,
  projectNodeDeletionExecuteBodySchema,
  projectNodeParamsSchema,
  projectParamsSchema,
...
export interface RegisterSystemRoutesOptions {
  ...
  projectNodeDeletionService: ProjectNodeDeletionService;
  workflowRuntimeActionsService: WorkflowRuntimeActionsService;
  inspectionsProcess: InspectionsProcess;
  importsExportsProcess: ImportsExportsProcess;
  startupService: StartupService;
}

export async function registerSystemRoutes(
  app: FastifyInstance,
  options: RegisterSystemRoutesOptions,
): Promise<void> {
  app.post('/api/projects/:projectId/deletion-check', async (request) => {
    ...
    return presentProjectDeletionCheckEnvelope(
      await options.projectDeletionService.checkProjectDeletion(projectId),
    );
  });
  ...
  app.post('/api/projects/:projectId/deletion-execute', async (request) => {
    ...
    return presentProjectDeletionExecuteEnvelope(
      await options.projectDeletionService.executeProjectDeletion(projectId, body),
    );
  });
  ...
  app.post('/api/project-nodes/:projectNodeId/deletion-check', async (request) => {
    ...
    parseInput(emptyBodySchema, request.body ?? {});
    return presentProjectNodeDeletionCheckEnvelope(
      await options.projectNodeDeletionService.checkProjectNodeDeletion(projectNodeId),
    );
  });
  ...
  app.post('/api/project-nodes/:projectNodeId/deletion-execute', async (request) => {
    ...
    return presentProjectNodeDeletionExecuteEnvelope(
      await options.projectNodeDeletionService.executeProjectNodeDeletion(projectNodeId, body),
    );
  });
  ...
  app.get('/api/project-nodes/:projectNodeId/workflow-runtime/nodes/:mermaidNodeId', async (request) => {
    ...
    return presentWorkflowRuntimeDetailEnvelope(
      await options.workflowRuntimeActionsService.getWorkflowRuntimeNodeDetail(projectNodeId, mermaidNodeId),
    );
  });
  ...
  app.post('/api/project-nodes/:projectNodeId/workflow-runtime/nodes/:mermaidNodeId/trigger', async (request) => {
    const { projectNodeId, mermaidNodeId } = parseInput(workflowRuntimeParamsSchema, request.params);
    parseInput(emptyBodySchema, request.body ?? {});
    return presentWorkflowRuntimeTriggerEnvelope(
      await options.workflowRuntimeActionsService.triggerWorkflowRuntimeNodeAction(projectNodeId, mermaidNodeId),
    );
  });
