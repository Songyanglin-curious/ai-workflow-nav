...
function readScriptOrder(fileName) {
...
function readDeclaredTableNames(sql) {
...
function assertScriptOrder(fileNames) {
    if (fileNames.length === 0) {
        throw new Error('schema 目录下未找到可执行的 SQL 脚本。');
    }
    const duplicatedOrder = new Set();
...
function readExistingTableNames(database) {
...
function assertDeclaredTablesCreated(database, scripts) {
...
export function resolveSchemaDirectoryPath(workspaceRootPath) {
    return path.resolve(workspaceRootPath, defaultSchemaRelativePath);
}
export async function readMigrationScripts(schemaDirectoryPath) {
...
export async function runMigrations(database, schemaDirectoryPath) {
    try {
