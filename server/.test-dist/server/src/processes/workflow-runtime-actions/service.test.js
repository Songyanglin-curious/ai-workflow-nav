...
function createExternalToolsStub() {
    return {
        toolExists() {
            return false;
        },
...
async function createPromptRuntimeFixture() {
    ...
    const runtimeService = createWorkflowRuntimeActionsService(context.database, context.workspacePaths, createExternalToolsStub());
    return {
        context,
        prompt,
        projectNode,
...
test('WorkflowRuntimeActionsService 可返回并触发 prompt 类型动作', async () => {
    ...
    try {
        ...
        const triggerResult = await fixture.runtimeService.triggerWorkflowRuntimeNodeAction(fixture.projectNode.id, 'START');
...
test('WorkflowRuntimeActionsService 会把缺失 prompt 文件视为 PromptNotFoundError', async () => {
    const fixture = await createPromptRuntimeFixture();
    try {
        await removePath(resolvePromptPath(fixture.context.workspacePaths, `${fixture.prompt.name}__${fixture.prompt.id.replaceAll('-', '').slice(0, 8)}.md`));
        const detail = await fixture.runtimeService.getWorkflowRuntimeNodeDetail(fixture.projectNode.id, 'START');
        assert.equal(detail.action?.isExecutable, false);
        assert.equal(detail.action?.failureReason, 'prompt_not_found');
        await assert.rejects(async () => fixture.runtimeService.triggerWorkflowRuntimeNodeAction(fixture.projectNode.id, 'START'), PromptNotFoundError);
    }
    finally {
        await fixture.context.cleanup();
    }
