...
import {
  presentAppendLatestDeliberationsEnvelope,
  presentCreateDeliberationsFileEnvelope,
  presentDeliberationsFilesEnvelope,
  presentDeliberationsFolderEnvelope,
  presentProjectDetailEnvelope,
  presentProjectNodeDetailEnvelope,
  presentProjectNodeLayoutsEnvelope,
  presentProjectNodeLayoutsPatchEnvelope,
  presentProjectNodeListEnvelope,
  presentProjectViewportEnvelope,
  presentProjectListEnvelope,
  presentSummaryFilesEnvelope,
  presentSummaryFolderEnvelope,
} from '../presenter/projects.js';
import {
  appendLatestDeliberationsBodySchema,
  createDeliberationsFileBodySchema,
  createProjectBodySchema,
  createProjectNodeBodySchema,
  patchProjectNodeLayoutsBodySchema,
  patchProjectViewportBodySchema,
  projectListQuerySchema,
  projectNodeParamsSchema,
  projectNodesListQuerySchema,
  projectParamsSchema,
  updateProjectBodySchema,
  updateProjectNodeBodySchema,
} from '../schema/projects.js';
import { parseInput } from './utils.js';
import type {
  DeliberationsService,
  ProjectNodeService,
  ProjectService,
  ProjectViewConfigService,
  SummaryService,
...
export interface RegisterProjectRoutesOptions {
  ...
  deliberationsService: DeliberationsService;
  summaryService: SummaryService;
  projectViewConfigService: ProjectViewConfigService;
}
...
export async function registerProjectRoutes(
  ...
  app.patch('/api/projects/:projectId/node-layouts', async (request) => {
    ...
    return presentProjectNodeLayoutsPatchEnvelope(
      options.projectViewConfigService.patchProjectNodeLayouts(projectId, body),
    );
  });
  ...
  app.get('/api/project-nodes/:projectNodeId/deliberations-records', async (request) => {
    const { projectNodeId } = parseInput(projectNodeParamsSchema, request.params);
    return presentDeliberationsFolderEnvelope(
      await options.deliberationsService.getDeliberationsRecordFolderInfo(projectNodeId),
    );
  });

  app.get('/api/project-nodes/:projectNodeId/deliberations-records/files', async (request) => {
    const { projectNodeId } = parseInput(projectNodeParamsSchema, request.params);
    return presentDeliberationsFilesEnvelope(
      await options.deliberationsService.listDeliberationsRecordFiles(projectNodeId),
    );
  });
  ...
  app.post('/api/project-nodes/:projectNodeId/deliberations-records/append-latest', async (request) => {
    ...
    return presentAppendLatestDeliberationsEnvelope(
      await options.deliberationsService.appendLatestDeliberations(projectNodeId, body),
    );
  });
