...
export interface CreateInspectionsProcessOptions {
  mustBindWorkflowNodeAction?: boolean;
}
...
function createWorkflowNodeIds(mermaidSource: string): Set<string> {
...
function createIssueSummary(items: InspectionIssue[]): InspectionRunResult['summary'] {
  let errorCount = 0;
  let warningCount = 0;
...
export function createInspectionsProcess(
  database: SqliteDatabase,
  workspacePaths: WorkspacePaths,
  options: CreateInspectionsProcessOptions = {},
) {
  return {
    async runInspection(): Promise<InspectionRunResult> {
      ...
      for (const prompt of prompts) {
        ...
        items.push(
          createIssue(
            ...
            prompt.id,
            '提示词正文文件不存在。',
            promptIndexedFileMissingSuggestion,
          ),
        );
      }
      ...
      for (const fileName of promptFileNames) {
        items.push(
          createIssue(
            ...
            'prompt',
            null,
            `提示词正文文件存在但未建立索引：${fileName}`,
            promptUnindexedFileSuggestion,
          ),
        );
      }
      ...
      const workflowNodeIdsById = new Map(
        workflows.map((workflow) => [workflow.id, createWorkflowNodeIds(workflow.mermaid_source)]),
      );

      const workflowNodeActionRows = database
        .prepare<WorkflowNodeActionRow>(`
          SELECT
            id,
            workflow_id,
            mermaid_node_id,
        ...
        if (!workflowNodeIds || !workflowNodeIds.has(nodeAction.mermaid_node_id)) {
          items.push(
            createIssue(
              ...
              nodeAction.id,
              `工作流节点动作已失效：${nodeAction.workflow_id} / ${nodeAction.mermaid_node_id}`,
              workflowNodeActionStaleSuggestion,
            ),
          );
          continue;
        }
          ...
          if (!promptExists) {
            items.push(
              createIssue(
                ...
                'workflow_node_action',
                nodeAction.id,
                `工作流节点动作引用的提示词不存在：${nodeAction.target_ref}`,
                bindingTargetMissingSuggestion,
              ),
            );
          }
        ...
        if (!toolKeys.has(nodeAction.target_ref)) {
          items.push(
            createIssue(
              ...
              'workflow_node_action',
              nodeAction.id,
              `工作流节点动作引用的本地工具不存在：${nodeAction.target_ref}`,
              toolTargetMissingSuggestion,
            ),
          );
        }
      }
      ...
      const projectNodeWorkflows = database
        .prepare<ProjectNodeWorkflowRow>(`
          SELECT
            id,
            project_node_id,
            workflow_id
      ...
      for (const binding of projectNodeWorkflows) {
        ...
        items.push(
          createIssue(
            'BINDING_TARGET_NOT_FOUND',
            'error',
            'project_node_workflow',
            binding.id,
            `项目节点工作流绑定引用的工作流不存在：${binding.workflow_id}`,
            bindingTargetMissingSuggestion,
          ),
        );
      }
...
export type InspectionsProcess = ReturnType<typeof createInspectionsProcess>;
