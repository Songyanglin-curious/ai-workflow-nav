...
4. 若非空，要求二次确认
5. 若选择转存，先转存再删除
6. 若存在直接子节点，先将其改写为根层孤岛节点
7. 将这些直接子节点按原相对顺序连续追加到根层末尾，并分配确定的根层 `sortOrder`
8. 删除当前节点自身
9. 若转存或孤岛改写失败，中止删除
...
3. `chatLogs/` 与 `summaries/` 必须拆成两个模块，避免规则混淆。
