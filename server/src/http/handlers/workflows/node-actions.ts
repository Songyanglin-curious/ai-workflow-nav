...
import {
  presentWorkflowNodeActionDeleteEnvelope,
  presentWorkflowNodeActionEnvelope,
  presentWorkflowNodeActionListEnvelope,
  presentWorkflowNodeActionSyncEnvelope,
} from '../../presenter/workflows.js';
import {
  createWorkflowNodeActionBodySchema,
  updateWorkflowNodeActionBodySchema,
  workflowNodeActionParamsSchema,
  workflowNodeActionSyncBodySchema,
} from '../../schema/workflows.js';
import type { WorkflowNodeActionService } from '../../../domains/workflows/node-actions/service.js';

export interface RegisterWorkflowNodeActionRoutesOptions {
  workflowNodeActionService: WorkflowNodeActionService;
}
...
function mapWorkflowNodeActionError(error: unknown): HttpMappedError {
...
function mapValidationIssues(error: unknown): { issues: Array<{ path: string; message: string; code?: string }> } {
  if (typeof error !== 'object' || error === null || !('issues' in error)) {
    return { issues: [] };
  }

  const issues = (error as { issues: Array<{ path: Array<string | number>; message: string; code?: string }> }).issues;
...
function replyWithWorkflowNodeActionError(reply: FastifyReply, error: unknown): FastifyReply {
  ...
  return reply.status(mappedError.statusCode).send(
    presentError(
      mappedError.code,
      mappedError.message,
      mappedError.statusCode === 422 ? mapValidationIssues(error) : undefined,
    ),
  );
}

export async function registerWorkflowNodeActionRoutes(
  app: FastifyInstance,
  options: RegisterWorkflowNodeActionRoutesOptions,
): Promise<void> {
  app.get('/api/workflows/:workflowId/node-actions', async (request, reply) => {
    ...
    if (!parsedParams.success) {
      return reply.status(422).send(
        presentError('VALIDATION_ERROR', '工作流节点动作列表参数校验失败。', mapValidationIssues(parsedParams.error)),
      );
    }
  ...
  app.post('/api/workflows/:workflowId/node-actions', async (request, reply) => {
    ...
    if (!parsedParams.success) {
      return reply.status(422).send(
        presentError('VALIDATION_ERROR', '工作流节点动作创建路径参数校验失败。', mapValidationIssues(parsedParams.error)),
      );
    }
    ...
    if (!parsedBody.success) {
      return reply.status(422).send(
        presentError('VALIDATION_ERROR', '工作流节点动作创建请求校验失败。', mapValidationIssues(parsedBody.error)),
      );
    }
  ...
  app.patch('/api/workflows/:workflowId/node-actions/:mermaidNodeId', async (request, reply) => {
    ...
    if (!parsedParams.success) {
      return reply.status(422).send(
        presentError('VALIDATION_ERROR', '工作流节点动作更新路径参数校验失败。', mapValidationIssues(parsedParams.error)),
      );
    }
    ...
    if (!parsedBody.success) {
      return reply.status(422).send(
        presentError('VALIDATION_ERROR', '工作流节点动作更新请求校验失败。', mapValidationIssues(parsedBody.error)),
      );
    }

    try {
      const nodeAction = options.workflowNodeActionService.updateNodeActionByWorkflowIdAndMermaidNodeId(
        parsedParams.data.workflowId,
        parsedParams.data.mermaidNodeId,
        parsedBody.data,
      );
  ...
  app.delete('/api/workflows/:workflowId/node-actions/:mermaidNodeId', async (request, reply) => {
    ...
    if (!parsedParams.success) {
      return reply.status(422).send(
        presentError('VALIDATION_ERROR', '工作流节点动作删除路径参数校验失败。', mapValidationIssues(parsedParams.error)),
      );
    }

    try {
      await options.workflowNodeActionService.deleteNodeActionByWorkflowIdAndMermaidNodeId(
        parsedParams.data.workflowId,
        parsedParams.data.mermaidNodeId,
      );
  ...
  app.post('/api/workflows/:workflowId/node-actions/sync', async (request, reply) => {
    ...
    if (!parsedParams.success) {
      return reply.status(422).send(
        presentError('VALIDATION_ERROR', '工作流节点动作同步路径参数校验失败。', mapValidationIssues(parsedParams.error)),
      );
    }
    ...
    if (!parsedBody.success) {
      return reply.status(422).send(
        presentError('VALIDATION_ERROR', '工作流节点动作同步请求校验失败。', mapValidationIssues(parsedBody.error)),
      );
    }
