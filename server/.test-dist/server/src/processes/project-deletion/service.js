...
import { ProjectDeletionBlockedError, ProjectDeletionConfirmationRequiredError, ProjectDeletionExecutionFailedError, ProjectSummaryArchiveFailedError, } from './errors.js';
import { createProjectDeletionRepo } from './repo.js';
async function countSummaryNodes(workspacePaths, archiveTargets) {
...
function createCheckResult(snapshot, summaryNodeCount) {
...
async function archiveProjectSummaries(workspacePaths, archiveTargets) {
...
async function restoreProjectSummaries(archiveRecords) {
...
export function createProjectDeletionService(database, workspacePaths) {
    const repo = createProjectDeletionRepo(database);
    return {
        async checkProjectDeletion(projectId) {
        ...
        async executeProjectDeletion(projectId, input) {
            ...
            const archiveRecords = input.strategy === 'archive_then_delete'
