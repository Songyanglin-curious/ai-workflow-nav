...
import {
  presentWorkflowDeleteEnvelope,
  presentWorkflowDetailEnvelope,
  presentWorkflowListEnvelope,
  presentWorkflowUpdateEnvelope,
} from '../presenter/workflows.js';
import {
  createWorkflowBodySchema,
  updateWorkflowBodySchema,
  workflowListQueryHttpSchema,
...
function mapWorkflowError(error: unknown): HttpMappedError {
...
function replyWithWorkflowError(reply: FastifyReply, error: unknown): FastifyReply {
...
export async function registerWorkflowRoutes(
  ...
  app.get('/api/workflows', async (request, reply) => {
    ...
    if (!parsedQuery.success) {
      return reply.status(422).send(
        presentError('VALIDATION_ERROR', '工作流列表查询参数校验失败。', mapValidationIssues(parsedQuery.error)),
      );
    }
  ...
  app.get('/api/workflows/:id', async (request, reply) => {
    ...
    if (!parsedParams.success) {
      return reply.status(422).send(
        presentError('VALIDATION_ERROR', '工作流路径参数校验失败。', mapValidationIssues(parsedParams.error)),
      );
    }
  ...
  app.post('/api/workflows', async (request, reply) => {
    ...
    if (!parsedBody.success) {
      return reply.status(422).send(
        presentError('VALIDATION_ERROR', '工作流创建请求校验失败。', mapValidationIssues(parsedBody.error)),
      );
    }
  ...
  app.patch('/api/workflows/:id', async (request, reply) => {
    ...
    if (!parsedBody.success) {
      return reply.status(422).send(
        presentError('VALIDATION_ERROR', '工作流更新请求校验失败。', mapValidationIssues(parsedBody.error)),
      );
    }
